...
export const GridContainer = ({
    ...
    <StyledContainer removeMobilePadding={removeMobilePadding} {...props}>
        {children}
    </StyledContainer>
)
...
const createGridBreakpoints = (breakpoints, spacing) => {
    ...
    for (const size in breakpoints) {
        styles += `
            @media screen and (max-width: ${breakpoints[size].maxScreenWidth +
                "px" || "none"}) {
                max-width: ${breakpoints[size].minScreenWidth + "px" ||
                    breakpoints[size].minScreenWidth + spacing * 2};
            }
        `
    }
