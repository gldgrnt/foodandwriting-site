...
export const ParallaxFeaturedTopic = ({ topic }) => {
    const imgSrcSet =
        topic.recipeCategoryData.categoryOptions.coverPhoto.asset.fluid
            .srcSetWebp
    ...
    function doParallax(e) {
        ...
        const parallaxCentre =
            ParallaxContainer.offsetTop + ParallaxContainer.clientHeight / 2

        const ratio = windowCentre / parallaxCentre
        ParallaxImg.style.transform = `translateY(${transformOffset -
            transformOffset * ratio}%)`
    }
                ...
                <GridContainer>
                    <GridRow>
                        <GridCol cols={{ monitor: 3, desktop: 4 }}>
                            <SmallCaps size="small" color="white">
                                Featured topic
                            </SmallCaps>
                            <Title>
                                {topic.recipeCategoryData.title} and sentence
                                about
                            </Title>
                            <p>
                            ...
                            <InternalLink
                                to={`/${topic.recipeCategoryData.slug.current}`}
                                secondary
                            >
                                View posts
                            </InternalLink>
