...
export default ({ data: { category, allPosts } }) => {
    ...
    const filteredPosts = allPosts.nodes.filter(
        post => post.category._id === category._id
    )
    const posts = {
        nodes: filteredPosts.slice(0, 15),
    ...
    const handleMorePostsClick = () => {
        ...
        axios
            .get(
                `/.netlify/functions/get-posts?categoryId=${categoryId}&amount=${AMOUNT}&offset=${offset}`
            )
            .then(response =>
                setState(oldState => {
                    // Create array of current and loaded posts
                    const allPosts = [
                        ...oldState.postsOnPage,
                        ...response.data.posts,
                    ]
    ...
    return (
        <CategoryPage
            category={category}
            posts={state.postsOnPage}
            getMorePosts={handleMorePostsClick}
            showButton={state.postsOnPage.length < TOTAL}
        />
    )
}
